...
CreateBudgetDialog::CreateBudgetDialog(QWidget *parent)
    : QDialog(parent) {
    setupDialog();
}
...
QString CreateBudgetDialog::getCategory() const {
    return categoryEdit->text();
}

double CreateBudgetDialog::getLimit() const {
    return limitSpinBox->value();
}

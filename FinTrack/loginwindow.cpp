...
double getTotalExpenses(int userId, const QString &category) {
    QSqlQuery query;
    query.prepare("SELECT SUM(amount) FROM transactions WHERE user_id = :user_id AND category = :category AND type = 'expense'");
    query.bindValue(":user_id", userId);
    query.bindValue(":category", category);
...
void LoginWindow::on_addExpenseButton_clicked() {
    ...
    if (expenseDialog->exec() == QDialog::Accepted) {
        ...
        QSqlQuery query;
        query.prepare("INSERT INTO transactions (user_id, type, category, amount, date, description) "
...
void LoginWindow::on_filterButton_clicked() {
    ...
    if (filterDialog.exec() == QDialog::Accepted) {
        ...
        QSqlQuery query;
        query.prepare("SELECT type, category, amount, date, description FROM transactions "
